...
return ( <header className="sticky top-0 z-40 border-b hairline bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/85"> <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-3 px-4 sm:px-6"> <SakanLogo /> <div className="flex items-center gap-2"> <LanguageSelector />
{authed ? ( <Button asChild size="sm" className="gold-surface font-semibold text-black hover:opacity-90"> <Link to="/discover"> <Compass className="me-1.5 h-4 w-4" />
{t('cta_enter')} </Link> </Button>
) : (
...
return (
<section aria-label={t('ads_title')}> <div className="mb-2.5 flex items-center gap-2">
<Megaphone className={`h-4 w-4 ${GOLD}`} /> <h2 className="text-sm font-semibold tracking-wide">
{t('ads_title')} </h2> </div>
...
return ( <section className="hero-glow relative overflow-hidden rounded-2xl border border-[hsl(43_78%_58%/0.45)] bg-card p-6 sm:p-8"> <div className="pointer-events-none absolute inset-0 bg-gradient-to-r from-transparent via-[hsl(43_78%_58%/0.06)] to-transparent" />
    ...
    <div className="flex items-start gap-4">
      ...
      <div>
        <p className="text-base font-bold leading-snug sm:text-lg">
          {t('promo_title')}{' '}
          <span className={GOLD}>— {t('promo_sub')}</span>
        </p>
...
return ( <section> <div className="mb-2.5 flex items-center gap-2">
<Crown className={`h-4 w-4 ${GOLD}`} /> <h2 className="text-sm font-semibold">{t('featured_title')}</h2> </div>
      ...
      {members.map((member) => (
        ...
        >
          <MemberAvatar
            ...
            premium
            className={cn(
              'h-16 w-16 transition-transform group-hover:scale-105',
              GOLD_RING
            )}
          />
...
return ( <nav className="overflow-x-auto rounded-xl border hairline bg-card [scrollbar-width:thin]"> <div className="flex min-w-max divide-x divide-[hsl(220_30%_18%)] rtl:divide-x-reverse">
{TABS.map(({ key, icon: Icon }) => {
const isActive =
key === active || (key === 'home' && active === 'home');
...
const links: Array<{
key: 'sidebar_edit' | 'sidebar_photos' | 'sidebar_settings';
icon: LucideIcon;
to: string;
}> = [
...
return ( <aside className="flex w-full flex-col gap-4 lg:w-60 lg:shrink-0"> <div className="rounded-xl border hairline bg-card p-4"> <p className="mb-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground">
{t('sidebar_menu')} </p>
...
```
    {me?.profile ? (
      ...
      >
        <MemberAvatar
          objectKey={me.profile.avatar_object_key}
        ...
        <span className="min-w-0">
          ...
          <span className="block text-xs text-muted-foreground">
            {flagFor(me.profile.country)} {me.profile.country}
          </span>
        </span>
      </Link>
    ) : (
  ...
  <div className="rounded-xl border hairline bg-card p-2">
    {links.map(({ key, icon: Icon, to }) => (
      <button
        ...
        type="button"
        onClick={() =>
          me ? navigate(to) : client.auth.toLogin()
        }
...
profile.is_premium
? 'border-[hsl(43_78%_58%/0.4)]'
: 'hairline'
)}
> <div className="relative aspect-[4/5] w-full overflow-hidden bg-secondary"> <MemberAvatar
...
sakanApi
  .me()
  .then((state) =>
    setFeaturedSelf(Boolean(state.is_featured))
  )
  .catch(() => undefined);
```
...
return ( <div className="min-h-screen bg-background"> <HomeHeader me={me} />
  ...
  <main className="mx-auto max-w-7xl px-4 pb-16 pt-5 sm:px-6">
    {loadingHome ? (
      ...
      <div className="space-y-6">
        ...
        <PromoBanner
          me={me}
          isFeatured={featuredSelf}
        />

        {home && (
          <FeaturedRibbon members={home.featured} />
        )}
            ...
            {tabLoading ? (
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 xl:grid-cols-4">
                {Array.from({ length: 8 }).map((_, i) => (
                  <div
                    key={i}
                    className="aspect-[4/5] animate-pulse rounded-xl bg-secondary/60"
                  />
                ))}
              </div>
            ) : members.length === 0 ? (
