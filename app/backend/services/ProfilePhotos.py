...
class ProfilephotosService:
    """Service layer for Profilephotos operations"""
    ...
    async def create(self, data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Profilephotos]:
        """Create a new ProfilePhotos"""
        try:
            if user_id:
                data['user_id'] = user_id
            obj = Profilephotos(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created ProfilePhotos with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating ProfilePhotos: {str(e)}")
            raise
    ...
    async def check_ownership(self, obj_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking ownership for ProfilePhotos {obj_id}: {str(e)}")
            return False

    async def get_by_id(self, obj_id: int, user_id: Optional[str] = None) -> Optional[Profilephotos]:
        """Get ProfilePhotos by ID (user can only see their own records)"""
        try:
            query = select(Profilephotos).where(Profilephotos.id == obj_id)
            if user_id:
                query = query.where(Profilephotos.user_id == user_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching ProfilePhotos {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of ProfilePhotoss (user can only see their own records)"""
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(Profilephotos.user_id == user_id)

            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Profilephotos, field):
                        column = getattr(Profilephotos, field)
            ...
            stmt = select(Profilephotos, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Profilephotos, field_name):
                        stmt = stmt.order_by(getattr(Profilephotos, field_name).desc())
                else:
                    if hasattr(Profilephotos, sort):
                        stmt = stmt.order_by(getattr(Profilephotos, sort))
            else:
                stmt = stmt.order_by(Profilephotos.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Profilephotos)
                for condition in conditions:
            ...
            logger.error(f"Error fetching ProfilePhotos list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Profilephotos]:
        """Update ProfilePhotos (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Profilephotos {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated ProfilePhotos {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating ProfilePhotos {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int, user_id: Optional[str] = None) -> bool:
        """Delete ProfilePhotos (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Profilephotos {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted ProfilePhotos {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting ProfilePhotos {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Profilephotos]:
        """Get ProfilePhotos by any field"""
        try:
            if not hasattr(Profilephotos, field_name):
                raise ValueError(f"Field {field_name} does not exist on Profilephotos")
            column = getattr(Profilephotos, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Profilephotos).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching ProfilePhotos by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Profilephotos]:
        """Get list of ProfilePhotoss filtered by field"""
        try:
            if not hasattr(Profilephotos, field_name):
                raise ValueError(f"Field {field_name} does not exist on Profilephotos")
            column = getattr(Profilephotos, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Profilephotos.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching ProfilePhotoss by {field_name}: {str(e)}")
            raise