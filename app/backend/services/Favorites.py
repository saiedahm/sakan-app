...
class FavoritesService:
    ...
    async def get_list(
        ...
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(Favorites.user_id == user_id)

            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Favorites, field):
                        column = getattr(Favorites, field)
            ...
            stmt = select(Favorites, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(Favorites, field_name):
                        stmt = stmt.order_by(getattr(Favorites, field_name).desc())
                else:
                    if hasattr(Favorites, sort):
                        stmt = stmt.order_by(getattr(Favorites, sort))
            else:
                stmt = stmt.order_by(Favorites.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Favorites)
                for condition in conditions:
    ...
    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Favorites]:
        ...
        try:
            ...
            column = getattr(Favorites, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Favorites).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
