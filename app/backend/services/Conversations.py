...
class ConversationsService:
    ...
    async def get_list(
        ...
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(Conversations.user_id == user_id)

            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Conversations, field):
                        column = getattr(Conversations, field)
            ...
            stmt = select(Conversations, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(Conversations, field_name):
                        stmt = stmt.order_by(getattr(Conversations, field_name).desc())
                else:
                    if hasattr(Conversations, sort):
                        stmt = stmt.order_by(getattr(Conversations, sort))
            else:
                stmt = stmt.order_by(Conversations.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Conversations)
                for condition in conditions:
    ...
    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Conversations]:
        ...
        try:
            ...
            column = getattr(Conversations, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Conversations).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
