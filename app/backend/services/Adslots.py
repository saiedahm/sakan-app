...
class AdslotsService:
    """Service layer for Adslots operations"""
    ...
    async def create(self, data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Adslots]:
        ...
        try:
            ...
            logger.info(f"Created Adslots with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating Adslots: {str(e)}")
            raise
    ...
    async def check_ownership(self, obj_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking ownership for Adslots {obj_id}: {str(e)}")
            return False

    async def get_by_id(self, obj_id: int, user_id: Optional[str] = None) -> Optional[Adslots]:
        """Get Adslots by ID (user can only see their own records)"""
        try:
            query = select(Adslots).where(Adslots.id == obj_id)
            if user_id:
                query = query.where(Adslots.user_id == user_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching Adslots {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of Adslotss (user can only see their own records)"""
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(Adslots.user_id == user_id)

            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Adslots, field):
                        column = getattr(Adslots, field)
            ...
            stmt = select(Adslots, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Adslots, field_name):
                        stmt = stmt.order_by(getattr(Adslots, field_name).desc())
                else:
                    if hasattr(Adslots, sort):
                        stmt = stmt.order_by(getattr(Adslots, sort))
            else:
                stmt = stmt.order_by(Adslots.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Adslots)
                for condition in conditions:
            ...
            logger.error(f"Error fetching Adslots list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Adslots]:
        """Update Adslots (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Adslots {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated Adslots {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating Adslots {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int, user_id: Optional[str] = None) -> bool:
        """Delete Adslots (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Adslots {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted Adslots {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting Adslots {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Adslots]:
        """Get Adslots by any field"""
        try:
            if not hasattr(Adslots, field_name):
                raise ValueError(f"Field {field_name} does not exist on Adslots")
            column = getattr(Adslots, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Adslots).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching Adslots by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Adslots]:
        """Get list of Adslotss filtered by field"""
        try:
            if not hasattr(Adslots, field_name):
                raise ValueError(f"Field {field_name} does not exist on Adslots")
            column = getattr(Adslots, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Adslots.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching Adslotss by {field_name}: {str(e)}")
            raise