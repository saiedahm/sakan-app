...
class PlansService:
    ...
    async def get_list(
        ...
        try:
            ...
            stmt = select(Plans, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(Plans, field_name):
                        stmt = stmt.order_by(getattr(Plans, field_name).desc())
                else:
                    if hasattr(Plans, sort):
                        stmt = stmt.order_by(getattr(Plans, sort))
            else:
                stmt = stmt.order_by(Plans.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Plans)
                for condition in conditions:
