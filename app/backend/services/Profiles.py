...
class ProfilesService:
    ...
    async def get_list(
        ...
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(Profiles.user_id == user_id)

            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Profiles, field):
                        column = getattr(Profiles, field)
            ...
            stmt = select(Profiles, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(Profiles, field_name):
                        stmt = stmt.order_by(getattr(Profiles, field_name).desc())
                else:
                    if hasattr(Profiles, sort):
                        stmt = stmt.order_by(getattr(Profiles, sort))
            else:
                stmt = stmt.order_by(Profiles.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Profiles)
                for condition in conditions:
    ...
    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Profiles]:
        ...
        try:
            ...
            column = getattr(Profiles, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Profiles).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
