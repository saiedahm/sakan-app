...
class PresencesService:
    """Service layer for Presences operations"""
    ...
    async def create(self, data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Presences]:
        ...
        try:
            ...
            logger.info(f"Created Presences with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating Presences: {str(e)}")
            raise
    ...
    async def check_ownership(self, obj_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking ownership for Presences {obj_id}: {str(e)}")
            return False

    async def get_by_id(self, obj_id: int, user_id: Optional[str] = None) -> Optional[Presences]:
        """Get Presences by ID (user can only see their own records)"""
        try:
            query = select(Presences).where(Presences.id == obj_id)
            if user_id:
                query = query.where(Presences.user_id == user_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching Presences {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of Presencess (user can only see their own records)"""
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(Presences.user_id == user_id)

            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Presences, field):
                        column = getattr(Presences, field)
            ...
            stmt = select(Presences, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Presences, field_name):
                        stmt = stmt.order_by(getattr(Presences, field_name).desc())
                else:
                    if hasattr(Presences, sort):
                        stmt = stmt.order_by(getattr(Presences, sort))
            else:
                stmt = stmt.order_by(Presences.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Presences)
                for condition in conditions:
            ...
            logger.error(f"Error fetching Presences list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Presences]:
        """Update Presences (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Presences {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated Presences {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating Presences {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int, user_id: Optional[str] = None) -> bool:
        """Delete Presences (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Presences {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted Presences {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting Presences {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Presences]:
        """Get Presences by any field"""
        try:
            if not hasattr(Presences, field_name):
                raise ValueError(f"Field {field_name} does not exist on Presences")
            column = getattr(Presences, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Presences).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching Presences by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Presences]:
        """Get list of Presencess filtered by field"""
        try:
            if not hasattr(Presences, field_name):
                raise ValueError(f"Field {field_name} does not exist on Presences")
            column = getattr(Presences, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Presences.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching Presencess by {field_name}: {str(e)}")
            raise