...
class ReportsService:
    ...
    async def get_list(
        ...
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(Reports.user_id == user_id)

            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Reports, field):
                        column = getattr(Reports, field)
            ...
            stmt = select(Reports, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(Reports, field_name):
                        stmt = stmt.order_by(getattr(Reports, field_name).desc())
                else:
                    if hasattr(Reports, sort):
                        stmt = stmt.order_by(getattr(Reports, sort))
            else:
                stmt = stmt.order_by(Reports.id.desc())
    ...
    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Reports]:
        ...
        try:
            ...
            column = getattr(Reports, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Reports).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
