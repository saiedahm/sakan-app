...
class PromotionsService:
    """Service layer for Promotions operations"""
    ...
    async def create(self, data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Promotions]:
        ...
        try:
            ...
            logger.info(f"Created Promotions with id: {obj.id}")
            return obj
    ...
    async def check_ownership(self, obj_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking ownership for Promotions {obj_id}: {str(e)}")
            return False

    async def get_by_id(self, obj_id: int, user_id: Optional[str] = None) -> Optional[Promotions]:
        """Get Promotions by ID (user can only see their own records)"""
        try:
            query = select(Promotions).where(Promotions.id == obj_id)
            if user_id:
                query = query.where(Promotions.user_id == user_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching Promotions {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of Promotionss (user can only see their own records)"""
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(Promotions.user_id == user_id)

            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Promotions, field):
                        column = getattr(Promotions, field)
            ...
            stmt = select(Promotions, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Promotions, field_name):
                        stmt = stmt.order_by(getattr(Promotions, field_name).desc())
                else:
                    if hasattr(Promotions, sort):
                        stmt = stmt.order_by(getattr(Promotions, sort))
            else:
                stmt = stmt.order_by(Promotions.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Promotions)
                for condition in conditions:
            ...
            logger.error(f"Error fetching Promotions list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Promotions]:
        """Update Promotions (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Promotions {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated Promotions {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating Promotions {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int, user_id: Optional[str] = None) -> bool:
        """Delete Promotions (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Promotions {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted Promotions {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting Promotions {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Promotions]:
        """Get Promotions by any field"""
        try:
            if not hasattr(Promotions, field_name):
                raise ValueError(f"Field {field_name} does not exist on Promotions")
            column = getattr(Promotions, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Promotions).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching Promotions by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Promotions]:
        """Get list of Promotionss filtered by field"""
        try:
            if not hasattr(Promotions, field_name):
                raise ValueError(f"Field {field_name} does not exist on Promotions")
            column = getattr(Promotions, field_name)
            ...
            logger.error(f"Error fetching Promotionss by {field_name}: {str(e)}")
            raise