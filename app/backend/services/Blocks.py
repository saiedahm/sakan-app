...
# ------------------ Service Layer ------------------
class BlocksService:
    """Service layer for Blocks operations"""
    ...
    async def create(self, data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Blocks]:
        """Create a new Blocks"""
        try:
            if user_id:
            ...
            logger.info(f"Created Blocks with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating Blocks: {str(e)}")
            raise
    ...
    async def check_ownership(self, obj_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking ownership for Blocks {obj_id}: {str(e)}")
            return False

    async def get_by_id(self, obj_id: int, user_id: Optional[str] = None) -> Optional[Blocks]:
        """Get Blocks by ID (user can only see their own records)"""
        try:
            query = select(Blocks).where(Blocks.id == obj_id)
            if user_id:
                query = query.where(Blocks.user_id == user_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching Blocks {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of Blockss (user can only see their own records)"""
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(Blocks.user_id == user_id)

            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Blocks, field):
                        column = getattr(Blocks, field)
            ...
            stmt = select(Blocks, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Blocks, field_name):
                        stmt = stmt.order_by(getattr(Blocks, field_name).desc())
                else:
                    if hasattr(Blocks, sort):
                        stmt = stmt.order_by(getattr(Blocks, sort))
            else:
                stmt = stmt.order_by(Blocks.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Blocks)
                for condition in conditions:
            ...
            logger.error(f"Error fetching Blocks list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Blocks]:
        """Update Blocks (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Blocks {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated Blocks {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating Blocks {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int, user_id: Optional[str] = None) -> bool:
        """Delete Blocks (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Blocks {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted Blocks {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting Blocks {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Blocks]:
        """Get Blocks by any field"""
        try:
            if not hasattr(Blocks, field_name):
                raise ValueError(f"Field {field_name} does not exist on Blocks")
            column = getattr(Blocks, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Blocks).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching Blocks by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Blocks]:
        """Get list of Blockss filtered by field"""
        try:
            if not hasattr(Blocks, field_name):
                raise ValueError(f"Field {field_name} does not exist on Blocks")
            column = getattr(Blocks, field_name)
            ...
            logger.error(f"Error fetching Blockss by {field_name}: {str(e)}")
            raise