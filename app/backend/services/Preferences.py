...
class PreferencesService:
    ...
    async def create(self, data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Preferences]:
        ...
        try:
            ...
            logger.info(f"Created Preferences with id: {obj.id}")
            return obj
    ...
    async def check_ownership(self, obj_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking ownership for Preferences {obj_id}: {str(e)}")
            return False

    async def get_by_id(self, obj_id: int, user_id: Optional[str] = None) -> Optional[Preferences]:
        """Get Preferences by ID (user can only see their own records)"""
        try:
            query = select(Preferences).where(Preferences.id == obj_id)
            if user_id:
                query = query.where(Preferences.user_id == user_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching Preferences {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of Preferencess (user can only see their own records)"""
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(Preferences.user_id == user_id)

            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Preferences, field):
                        column = getattr(Preferences, field)
            ...
            stmt = select(Preferences, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(Preferences, field_name):
                        stmt = stmt.order_by(getattr(Preferences, field_name).desc())
                else:
                    if hasattr(Preferences, sort):
                        stmt = stmt.order_by(getattr(Preferences, sort))
            else:
                stmt = stmt.order_by(Preferences.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Preferences)
                for condition in conditions:
            ...
            logger.error(f"Error fetching Preferences list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Preferences]:
        """Update Preferences (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Preferences {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated Preferences {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating Preferences {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int, user_id: Optional[str] = None) -> bool:
        """Delete Preferences (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Preferences {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted Preferences {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting Preferences {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Preferences]:
        """Get Preferences by any field"""
        try:
            if not hasattr(Preferences, field_name):
                raise ValueError(f"Field {field_name} does not exist on Preferences")
            column = getattr(Preferences, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Preferences).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching Preferences by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Preferences]:
        """Get list of Preferencess filtered by field"""
        try:
            if not hasattr(Preferences, field_name):
                raise ValueError(f"Field {field_name} does not exist on Preferences")
            column = getattr(Preferences, field_name)
            ...
            logger.error(f"Error fetching Preferencess by {field_name}: {str(e)}")
            raise