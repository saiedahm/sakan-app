...
class VisitsService:
    ...
    async def get_list(
        ...
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(Visits.user_id == user_id)

            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Visits, field):
                        column = getattr(Visits, field)
            ...
            stmt = select(Visits, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(Visits, field_name):
                        stmt = stmt.order_by(getattr(Visits, field_name).desc())
                else:
                    if hasattr(Visits, sort):
                        stmt = stmt.order_by(getattr(Visits, sort))
            else:
                stmt = stmt.order_by(Visits.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Visits)
                for condition in conditions:
