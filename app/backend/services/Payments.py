...
class PaymentsService:
    ...
    async def get_list(
        ...
        try:
            ...
            stmt = select(Payments, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(Payments, field_name):
                        stmt = stmt.order_by(getattr(Payments, field_name).desc())
                else:
                    if hasattr(Payments, sort):
                        stmt = stmt.order_by(getattr(Payments, sort))
            else:
                stmt = stmt.order_by(Payments.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Payments)
                for condition in conditions:
    ...
    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Payments]:
        ...
        try:
            ...
            column = getattr(Payments, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Payments).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
